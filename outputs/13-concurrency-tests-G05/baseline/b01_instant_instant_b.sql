...
-- If A committed first (arrival collapse), the raw trigger throws —
-- that raw-engine-error outcome is a PASS family too; TRY/CATCH keeps
-- the runner error-free (no stray sqlcmd exit).
-- ============================================================
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
-- overlapping (start_time = W1+30min, 1h) confirmed booking.
...
BEGIN TRY
    ...
    PRINT 'b01-B: committed overlapping confirmed booking ' + CAST(@b_b AS VARCHAR(12)) + ' (no control).';
END TRY
BEGIN CATCH
    PRINT 'PASS b01-B: RAW-ERROR family — overlap insert rejected by trigger (error '
...
ELSE
    PRINT 'b01-B: no overlap persisted (collapsed race; backstop path).';

-- Cleanup of this session's row (A cleans the other).
IF @b_b IS NOT NULL AND EXISTS (SELECT 1 FROM dbo.bookings WHERE booking_id = @b_b)
