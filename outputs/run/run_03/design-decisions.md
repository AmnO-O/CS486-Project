...
### Decision: Incident reporting merged into Maintenance entity (OVERRIDDEN — see run_01 amendment below)

**Task:** 2 (ERD Design)
**Date:** 2026-06-17 → **OVERRIDDEN 2026-07-04**
...
**⛔ OVERRIDDEN 2026-07-04 — See run_01 amendment below.**
...
### Decision: run_01 amendment — Separate Incidents entity (overrides Option B above)

**Task:** Run 01 (Design Amendment)
**Date:** 2026-07-04

**Problem:** Re-review of `req/business-requirement.md` revealed that incident reporting is a genuinely distinct concept with unique attributes:
- `incident_type` (theft, vandalism, accident, safety_hazard, policy_violation, lost_found, other)
- `severity` (low, medium, high, critical)
- `status` lifecycle: reported → investigating → resolved → closed

These attributes do not exist on the Maintenance entity, and merging them would violate SRP (Maintenance is for equipment repair, not general incident tracking).

**Options considered:**
- Option A (current): Keep incidents merged into Maintenance — loses incident-specific attributes, forces filtering on `problem_description`
- Option B (new): Separate `incidents` table with own PK, FKs to spaces and users, trigger-based auto-resolution via maintenance completion

**Decision:** We chose Option B because:
1. Incidents and maintenance are conceptually distinct (unexpected events vs. equipment repair)
2. Incidents have unique attributes (incident_type, severity) that justify a separate table
3. No `linked_maintenance_id` FK — instead use a trigger on maintenance UPDATE to auto-resolve incidents for the same space when maintenance completes
4. Unresolved incidents block booking (new trigger `trg_bookings_check_incidents`)
5. Total participation: all incidents are tied to a specific space (`space_id NOT NULL`)

**Impact:**
- New `incidents` table (10th entity, R12–R14)
- New triggers: `trg_incidents_autoresolve` (on maintenance UPDATE), `trg_bookings_check_incidents` (on bookings INSERT/UPDATE)
- New BRs: BR20 (incidents block booking), BR21 (maintenance auto-resolves incidents)
- All outputs updated in `outputs/run/run_01/`
- Previous `design-decisions.md` entry "Incident reporting merged into Maintenance entity" is overridden
...
| 2026-07-04 | Facility refactoring — added facility_categories, instance-level facilities, facility_id FK on maintenance/incidents, updated trigger | Agent | Run 01 amendment |

---

### Decision: run_01 amendment — Facility refactoring (instance-level tracking + prefix lookup table)

**Task:** Run 01 (Design Amendment)
**Date:** 2026-07-04

**Problem:** The original `facilities` table modeled equipment as category-level entries (one row per type: "Projector", "Whiteboard", "Computer"). `space_facilities` had a `quantity` column to count how many of each type were in a space. This prevented tracking individual devices (e.g., "Projector #3 in Lab 201 is broken") and making maintenance/incidents reference a specific physical asset.

**Options considered:**

- **Option A (original):** Keep category-level tracking — `facilities.name` as unique category, `space_facilities.quantity` for counts. Pros: simpler, fewer rows. Cons: cannot reference specific devices, cannot track per-device lifecycle.
- **Option B (new):** Refactor to instance-level tracking — introduce `facility_categories` lookup table for category/prefix mapping, convert `facilities` to store individual device instances (one row per physical device), remove `quantity` from `space_facilities` (each row = one device assignment), add nullable `facility_id` FK to `maintenance` and `incidents`.

**Decision:** We chose Option B because:
1. **Prefix lookup table** (`facility_categories`) provides a controlled vocabulary for device categories and their 3-letter prefix codes (pro, whb, mic, com, air, liv).
2. **Instance-level `facilities`** allows tracking individual device lifecycle (active, inactive, retired, lost) and enables precise linking via FK.
3. **`facility_id` FK on `maintenance`** lets staff report repairs for a specific device (e.g., "Projector pro_5 in Lab 201 needs repair") vs. the current space-level only.
4. **`facility_id` FK on `incidents`** lets reporters link incidents to specific equipment (e.g., "Microphone mic_3 in AUD-001 was stolen").
5. **`quantity` removed** from `space_facilities` — per-category counts derived via `COUNT(*) GROUP BY space_id, category_id`.
6. **Facility code** (`pro_1`, `whb_10`) derived at query time via `CONCAT(fc.prefix, '_', f.facility_id)` — no trigger needed for ID generation.
7. **`trg_incidents_autoresolve` updated** — when maintenance resolves with a `facility_id`, incidents for the same device auto-resolve (precise match); when maintenance has no `facility_id`, incidents for the same space auto-resolve (space-level fallback, previous behavior).

**Impact:**
- New `facility_categories` table (11th entity, R15)
- `facilities`: `name` column removed, replaced by `category_id FK → facility_categories` + `status` column
- `space_facilities`: `quantity` column removed — one row per physical device
- `maintenance`: new nullable `facility_id FK → facilities.facility_id` (R16)
- `incidents`: new nullable `facility_id FK → facilities.facility_id` (R17)
- `trg_incidents_autoresolve`: enhanced to resolve incidents by facility_id when applicable
- All 9 output files updated in `outputs/run/run_01/`
- Sample data grows from 6 facility categories to ~75 individual device rows

**Requirement reference:** `req/business-requirement.md` §3.2 (equipment management), `docs/project-overview.md` (device tracking)

### Decision: Add facility_assignments for time-range tracking (run_03)

**Task:** 3 (Logical Design)
**Date:** 2026-07-05

**Problem:** `facilities.space_id` only tracks the current location with no history. Future reservation planning (e.g., reserving specific equipment for an event) is impossible. There is no way to know where a device was located at a past date or where it will be in the future.

**Options considered:**
- Option A: Keep `facilities.space_id` as-is, add audit log table for changes (pros: simple; cons: no built-in planning support, separate audit)
- Option B: Add `facility_assignments` table with time ranges as source of truth; `facilities.space_id` demoted to cached column synced by app layer (pros: full history + future planning, 1 table instead of 2)

**Decision:** We chose Option B — add `facility_assignments` as the source of truth for facility location. `facilities.space_id` is kept as a denormalized cached column synced by the application layer (not DB triggers), so the app layer manages when to update current location based on active assignments.

**Status semantics:** `planned`/`active`/`completed` are time-derived and managed by app layer (planned→active→completed via periodic checks of start_time/end_time vs GETDATE()). `cancelled` is the only user-initiated business state.

**Impact:**
- Entity count: 10 → 11 (+Facility_Assignments)
- R6: Spaces→Facilities (1:N direct) → M:N bridge via Facility_Assignments (R18+R19)
- BR22 (no-overlap trigger) + BR23 (filtered UQ for 1 active/facility) added
- All downstream artifacts regenerated in `outputs/run/run_03/`

**Requirement reference:** `req/business-requirement.md` §3.2 (equipment management), user-expanded requirement for facility location history + reservation planning

---

### Decision: Remove space_facilities junction table (run_02)

**Task:** 3 (Logical Design)
**Date:** 2026-07-05

**Problem:** After refactoring Facilities to instance-level tracking (each row = one physical device), the `space_facilities` junction table (associating facilities to spaces) is no longer needed — a single physical device cannot be in multiple spaces simultaneously. The M:N relationship is semantically incorrect for this domain.

**Options considered:**
- Option A: Keep `space_facilities` junction table — one row per device assigned to a space (pros: consistent with ERD principles; cons: unnecessary indirection, no device can be M:N with spaces, FK cascade complexity)
- Option B: Move `space_id` directly into `facilities` table, nullable (pros: simpler schema — 10 tables instead of 11, direct 1:N, natural "in storage" representation via NULL; cons: schema change propagates through all downstream artifacts)

**Decision:** We chose Option B — drop `space_facilities` and add `space_id INT NULL FK → spaces.space_id` to `facilities`. This is semantically correct because:
1. A device can be in exactly one space (or storage) at a time — 1:N, not M:N
2. NULL `space_id` elegantly represents devices currently not assigned to any space
3. `SET NULL` on space delete automatically marks devices as "in storage"
4. One fewer table reduces query complexity and FK graph

**Impact:**
- R6 changed from Spaces ↔ Facilities (M:N) to Spaces → Facilities (1:N, partial via nullable space_id)
- Entity/schema registries updated: 10 entities, 10 tables (was 11)
- ERD, logical design, validation, DDL, sample data, queries all regenerated in `outputs/run/run_02/`
- All `space_facilities` JOINs replaced with direct `facilities.space_id` access
- Storage query: `SELECT ... FROM facilities WHERE space_id IS NULL`

**Requirement reference:** `req/business-requirement.md` §3.2 (equipment management — one device in one room at a time)
