...
-- Run 03: 11-table schema, +facility_assignments, R6 bridged through assignments
-- ============================================================
...
GO
DROP TABLE IF EXISTS facility_assignments;
GO
...
CREATE TABLE facility_categories (
    ...
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT PK_facility_categories PRIMARY KEY (category_id)
);
GO
...
CREATE TABLE facilities (
    ...
    CONSTRAINT FK_facilities_category FOREIGN KEY (category_id) REFERENCES facility_categories(category_id) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT FK_facilities_space FOREIGN KEY (space_id) REFERENCES spaces(space_id) ON DELETE SET NULL ON UPDATE NO ACTION,
    CONSTRAINT CK_facilities_status CHECK (status IN ('active', 'inactive', 'retired', 'lost'))
);
GO

-- ============================================================
-- 6. facility_assignments
-- ============================================================
CREATE TABLE facility_assignments (
    ...
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    updated_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT PK_facility_assignments PRIMARY KEY (assignment_id),
    CONSTRAINT CK_fac_assign_status CHECK (status IN ('planned','active','completed','cancelled')),
    CONSTRAINT CK_fac_assign_time CHECK (end_time > start_time),
    CONSTRAINT FK_fac_assign_facility FOREIGN KEY (facility_id) REFERENCES facilities(facility_id) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT FK_fac_assign_space FOREIGN KEY (space_id) REFERENCES spaces(space_id) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT FK_fac_assign_created_by FOREIGN KEY (created_by) REFERENCES users(user_id) ON DELETE NO ACTION ON UPDATE NO ACTION
);
GO
...
CREATE TABLE maintenance (
    ...
    CONSTRAINT FK_maintenance_facility FOREIGN KEY (facility_id) REFERENCES facilities(facility_id) ON DELETE SET NULL ON UPDATE NO ACTION
);
GO
...
CREATE TABLE incidents (
    ...
    CONSTRAINT CK_incidents_incident_type CHECK (incident_type IN ('theft','vandalism','accident','safety_hazard','policy_violation','lost_found','other')),
    CONSTRAINT CK_incidents_severity CHECK (severity IN ('low','medium','high','critical')),
    CONSTRAINT CK_incidents_status CHECK (status IN ('reported','investigating','resolved','closed')),
    CONSTRAINT FK_incidents_space_id FOREIGN KEY (space_id) REFERENCES spaces(space_id) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT FK_incidents_reported_by FOREIGN KEY (reported_by) REFERENCES users(user_id) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT FK_incidents_assigned_to FOREIGN KEY (assigned_to) REFERENCES users(user_id) ON DELETE SET NULL ON UPDATE NO ACTION,
    CONSTRAINT FK_incidents_facility FOREIGN KEY (facility_id) REFERENCES facilities(facility_id) ON DELETE SET NULL ON UPDATE NO ACTION
);
GO
...
-- facility_categories
CREATE UNIQUE NONCLUSTERED INDEX UQ_facility_categories_name ON facility_categories(category_name);
GO
CREATE UNIQUE NONCLUSTERED INDEX UQ_facility_categories_prefix ON facility_categories(prefix);
GO
...
GO
CREATE NONCLUSTERED INDEX idx_facilities_space_id ON facilities(space_id) WHERE space_id IS NOT NULL;
GO

-- facility_assignments
CREATE UNIQUE NONCLUSTERED INDEX UQ_fac_assign_active ON facility_assignments(facility_id) WHERE status = 'active';
GO
CREATE INDEX idx_fac_assign_facility_id ON facility_assignments (facility_id);
GO
CREATE INDEX idx_fac_assign_space_id ON facility_assignments (space_id);
GO
CREATE INDEX idx_fac_assign_time_range ON facility_assignments (facility_id, start_time, end_time);
GO
CREATE INDEX idx_fac_assign_created_by ON facility_assignments (created_by);
GO
...
GO
CREATE INDEX idx_maintenance_facility_id ON maintenance(facility_id) WHERE facility_id IS NOT NULL;
GO
...
GO
CREATE INDEX idx_incidents_facility_id ON incidents(facility_id) WHERE facility_id IS NOT NULL;
GO
...
-- BR21: Maintenance completion auto-resolves incidents (facility-aware)
GO
CREATE TRIGGER trg_incidents_autoresolve
ON maintenance
AFTER UPDATE
AS
BEGIN
    ...
    BEGIN
        ...
        SET i.status = 'resolved',
            i.resolved_at = m.completion_time,
            i.updated_at = SYSDATETIME()
        FROM incidents i
        INNER JOIN inserted m ON i.space_id = m.space_id
            AND (m.facility_id IS NOT NULL AND i.facility_id = m.facility_id)
        WHERE i.status IN ('reported', 'investigating')
            AND m.status = 'resolved'
            AND i.created_at < m.completion_time;
        ...
        WHERE i.status IN ('reported', 'investigating')
            ...
            AND i.created_at < m.completion_time
            AND NOT EXISTS (
                SELECT 1 FROM inserted m2 WHERE m2.facility_id IS NOT NULL
            );
    END
END
GO

-- BR22: Facility assignment no-overlap
GO
CREATE TRIGGER trg_facility_assignments_no_overlap
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
        FROM inserted i
        INNER JOIN facility_assignments fa
            ON fa.facility_id = i.facility_id
            AND fa.assignment_id <> i.assignment_id
            AND fa.status IN ('planned','active')
            AND fa.start_time < i.end_time
            AND fa.end_time > i.start_time
        WHERE i.status IN ('planned','active')
    )
    BEGIN
        RAISERROR('Overlapping time range exists for this facility.', 16, 1);
        ROLLBACK TRANSACTION;
        RETURN;
    END
END
...
GO
CREATE TRIGGER trg_facility_categories_updated_at
...
BEGIN
    ...
    BEGIN
        ...
        INNER JOIN inserted i ON fc.category_id = i.category_id;
    END
END
GO
...
GO
CREATE TRIGGER trg_facility_assignments_updated_at
ON facility_assignments
AFTER UPDATE
AS
BEGIN
    ...
    BEGIN
        ...
        INNER JOIN inserted i ON fa.assignment_id = i.assignment_id;
    END
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        SET updated_at = GETDATE()
        FROM incidents inc
        INNER JOIN inserted i ON inc.incident_id = i.incident_id;
    END
END
GO
