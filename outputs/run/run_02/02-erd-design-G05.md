...
The Campus Space Management System database centers on **Users**, **Spaces**, and **Bookings**, supported by organizational units (**Departments**), equipment **categories (Facility_Categories)**, and individual device instances (**Facilities**). The booking lifecycle is split across three dedicated tables: **Bookings** handles the initial request, **Booking_Approvals** captures the approval/rejection decision, and **Booking_Sessions** tracks check-in and completion. Maintenance requests and assignments are managed through the **Maintenance** entity, with device assignments tracked directly via `space_id` FK on **Facilities** (nullable for devices in storage). **Incidents** tracks unexpected events (theft, vandalism, accidents, safety hazards) tied to specific spaces, with trigger-based auto-resolution when related maintenance completes.
...
| R6 | Spaces → Facilities | 1:N | `Spaces \|\|--o{ Facilities` | A Space may contain zero or many Facility device instances; a Facility device must be assigned to exactly one Space, though assignment may be unknown or in storage (partial — `space_id` is nullable). |
...
- [x] All 10 entities from entity-registry are present (Departments, Users, Spaces, Facility_Categories, Facilities, Bookings, Booking_Approvals, Booking_Sessions, Maintenance, Incidents)
...
- [x] All 17 relationships from the Relationship Registry are present (R1–R17)
- [x] Cardinality is correct: 1:N for R1–R8, R12–R13, R15; 1:0..1 for R10–R11; 1:N partial for R9, R14, R16, R17
- [x] Participation constraints (`||` mandatory, `|o` optional) are explicitly stated and justified per Section 3
- [x] R6 is a direct 1:N relationship (Spaces → Facilities via `space_id` FK), no junction table needed
...
- [x] Entity count matches entity-registry (10 entities)
- [x] Key design decisions documented in `docs/design-decisions.md` (run_01 amendment — facility refactoring, run_02 amendment — remove space_facilities, building/floor, rejection reason, SRP split)
