...
**Date:** 2026-07-05
**Status:** Regenerated — 10-table schema, space_facilities junction removed, R6 changed to 1:N via nullable space_id FK in facilities (run_02)
...
**Scope:**
- 10 tables: `departments`, `users`, `spaces`, `facility_categories`, `facilities`, `bookings`, `booking_approvals`, `booking_sessions`, `maintenance`, `incidents`
...
| R6 | Spaces → Facilities | 1:N (partial) | FK in `facilities` (nullable) | space_id | spaces |
...
No table contains multi-valued attributes or repeating groups. Facilities directly reference spaces via `space_id` FK (1:N), no junction table needed.
...
**Conclusion:** All 10 tables satisfy 3NF.
...
| BR12 | Audit trail (created_at, updated_at) | Per-table columns | All 10 tables have `created_at DATETIME2 NOT NULL DEFAULT GETDATE()` and `updated_at DATETIME2 NOT NULL DEFAULT GETDATE()` | Database (DEFAULT) |
