...
WHERE s.building = @building
  ...
  AND s.current_status = @space_status_available
      AND EXISTS (
          ...
          INNER JOIN facility_categories fc ON f.category_id = fc.category_id
          WHERE sf.space_id = s.space_id
            AND fc.category_name = @facility_projector
      )
      AND EXISTS (
          SELECT 1
          FROM space_facilities sf
          ...
          INNER JOIN facility_categories fc ON f.category_id = fc.category_id
          WHERE sf.space_id = s.space_id
            AND fc.category_name = @facility_whiteboard
      )
  AND NOT EXISTS (
      SELECT 1
      FROM bookings b
...
WITH lab_inventory AS (
    SELECT
        ...
        s.capacity,
        s.current_status,
        COUNT(CASE WHEN fc.category_name = @computer_facility_name THEN f.facility_id ELSE NULL END) AS workstation_count,
        COUNT(CASE WHEN fc.category_name = @projector_facility_name THEN f.facility_id ELSE NULL END) AS projector_count
    FROM spaces s
...
SELECT
    ...
    STRING_AGG(CONCAT(fc.category_name, N' [', fc.prefix, N'_', f.facility_id, N']'), N', ') WITHIN GROUP (ORDER BY fc.category_name, f.facility_id) AS facility_summary
FROM bookings b
INNER JOIN spaces s
    ON s.space_id = b.space_id
INNER JOIN booking_sessions bs
...
-- ============================================================
-- Query 21: Active Incident Queue for Facility Staff
-- ============================================================
-- student-name: (run_01 amendment)
-- target-users: facility_staff
-- business-question:
--   Which incidents are currently active (reported or under
--   investigation), and what are their severity, type, affected
--   space, and assignment status? Facility staff need a
--   prioritized view to dispatch resources effectively.
-- ============================================================
...
SELECT
    ...
    inc.created_at,
    DATEDIFF(HOUR, inc.created_at, GETDATE()) AS hours_since_reported
FROM incidents inc
INNER JOIN spaces s
    ON s.space_id = inc.space_id
INNER JOIN users reporter
    ON reporter.user_id = inc.reported_by
LEFT JOIN users assignee
    ON assignee.user_id = inc.assigned_to
...
-- ============================================================
-- Query 22: Spaces Blocked by Unresolved Incidents
-- ============================================================
-- student-name: (run_01 amendment)
-- target-users: facility_staff, facility_manager
-- business-question:
--   Which spaces have unresolved incidents that are blocking
--   new bookings, and how many incidents per space remain open?
--   Facility managers need this to determine where to focus
--   incident resolution before approving incoming requests.
-- ============================================================
...
-- ============================================================
-- Query 23: Incident Resolution Time Analysis
-- ============================================================
-- student-name: (run_01 amendment)
-- target-users: facility_manager
-- business-question:
--   What is the average resolution time for incidents,
--   grouped by incident type and severity? The facility
--   manager needs this to identify which types of incidents
--   take longest to resolve and whether severity correlates
...
SELECT
    ...
    ROUND(AVG(CAST(DATEDIFF(HOUR, inc.created_at, inc.resolved_at) AS DECIMAL(10, 2))), 2) AS avg_resolution_hours,
    MIN(DATEDIFF(HOUR, inc.created_at, inc.resolved_at))             AS min_resolution_hours,
    MAX(DATEDIFF(HOUR, inc.created_at, inc.resolved_at))             AS max_resolution_hours
FROM incidents inc
WHERE inc.status IN ('resolved', 'closed')
  AND inc.resolved_at IS NOT NULL
  AND inc.created_at >= @semester_start
GROUP BY inc.incident_type, inc.severity
ORDER BY avg_resolution_hours DESC, inc.severity, inc.incident_type;
GO

-- ============================================================
-- Query 24: Incidents Auto-Resolved via Maintenance
-- ============================================================
-- student-name: (run_01 amendment)
-- target-users: facility_staff, facility_manager
-- business-question:
--   Which incidents were resolved through maintenance
--   completion (trigger-based auto-resolution), and what
--   maintenance work addressed them? This traces the
--   incident-to-maintenance resolution chain for audit
...
SELECT
    ...
    inc.incident_type,
    inc.severity,
    LEFT(inc.description, 200)               AS incident_description,
    inc.resolved_at,
...
-- ============================================================
-- Date       : 2026-07-04
-- Author     : (facility refactoring)
-- Reason     : Schema change — facilities table refactored from
--              category-level rows (name, quantity) to
--              per-device rows (category_id FK, status) with a
--              new facility_categories lookup table.
--
-- Changes:
--   Q11  : Replaced f.name with fc.category_name in both EXISTS
--          subqueries; added INNER JOIN facility_categories.
--   Q16  : Replaced SUM(CASE WHEN f.name=... THEN sf.quantity)
--          with COUNT(CASE WHEN fc.category_name=... THEN
--          f.facility_id); added LEFT JOIN facility_categories;
--          removed sf.quantity references.
--   Q18  : Replaced f.name/sf.quantity in facility_summary with
--          fc.category_name and fc.prefix + f.facility_id code;
--          added LEFT JOIN facility_categories.
-- ============================================================