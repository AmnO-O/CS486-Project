...
CREATE TABLE facility_categories (
    ...
    CONSTRAINT UQ_facility_categories_name UNIQUE (category_name),
    CONSTRAINT UQ_facility_categories_prefix UNIQUE (prefix)
);
GO
...
CREATE TABLE facilities (
    ...
    CONSTRAINT FK_facilities_category FOREIGN KEY (category_id) REFERENCES facility_categories(category_id),
    CONSTRAINT CK_facilities_status CHECK (status IN ('active', 'inactive', 'retired', 'lost'))
);
GO

-- ============================================================
-- 6. space_facilities (junction table)
-- ============================================================
CREATE TABLE space_facilities (
    ...
    CONSTRAINT FK_space_facilities_space FOREIGN KEY (space_id) REFERENCES spaces(space_id),
    CONSTRAINT FK_space_facilities_facility FOREIGN KEY (facility_id) REFERENCES facilities(facility_id)
);
GO
...
CREATE TABLE maintenance (
    ...
    CONSTRAINT FK_maintenance_facility FOREIGN KEY (facility_id) REFERENCES facilities(facility_id)
);
GO

-- ============================================================
-- 11. incidents (run_01 amendment)
-- ============================================================
CREATE TABLE incidents (
    ...
    CONSTRAINT FK_incidents_facility FOREIGN KEY (facility_id) REFERENCES facilities(facility_id)
);
GO
...
BEGIN
    ...
    IF UPDATE(status) AND EXISTS (SELECT 1 FROM inserted WHERE status = 'resolved')
    BEGIN
        -- Case 1: maintenance with facility_id → resolve incidents for that specific device
        UPDATE i
        ...
        -- Case 2: maintenance without facility_id → resolve all incidents in that space (fallback)
        UPDATE i
        SET i.status = 'resolved',
            i.resolved_at = m.completion_time,
            i.updated_at = SYSDATETIME()
        ...
        WHERE i.status IN ('reported', 'investigating')
            AND NOT EXISTS (
                SELECT 1 FROM inserted m2
                WHERE m2.facility_id IS NOT NULL AND m2.maintenance_id = m.maintenance_id
            );
    END
END
GO
