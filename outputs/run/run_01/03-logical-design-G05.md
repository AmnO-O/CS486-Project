...
**Date:** 2026-07-01
**Status:** Regenerated — 11-table schema with facility refactoring + SRP booking split + incidents (run_01 amendment)
...
**Scope:**
- 11 tables: `departments`, `users`, `spaces`, `facility_categories`, `facilities`, `space_facilities`, `bookings`, `booking_approvals`, `booking_sessions`, `maintenance`, `incidents`
- 17 relationships mapped (R1–R17)
- All business rules (BR1–BR21) enforced via constraints, indexes, and triggers
...
### 2.6 space_facilities (junction table)
...
Composite PK: (space_id, facility_id). This junction table resolves the M:N relationship between spaces and facility instances. Each row assigns one physical device instance to a room.
...
| R6 | Spaces ↔ Facilities | M:N | Junction table `space_facilities` (instance-level) | space_id, facility_id | spaces, facilities |
...
| space_facilities | (space_id, facility_id) composite | ✓ | No non-key attributes. Composite key alone; no partial dependency possible. |
...
| facilities | ✓ | All non-key attributes (`category_id`, `status`) depend solely on `facility_id`. `category_id` is a FK, not a transitive dependency. |
| space_facilities | ✓ | No non-key attributes. No transitive dependency. |
...
| BR20 | Unresolved incidents block booking | `trg_bookings_check_incidents` | AFTER INSERT, UPDATE trigger on `bookings` rejects if unresolved incidents exist for the same space with status IN ('reported','investigating') | Database (trigger) |
...
| `trg_bookings_check_incidents` | bookings INSERT, UPDATE | Reject if there exists an unresolved incident (status IN ('reported','investigating')) for the same space. Unresolved incidents make the space unavailable for booking regardless of `spaces.current_status`. |
