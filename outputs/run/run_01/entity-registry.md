...
> ✅ **All 11 entities below are 🔒 locked as of Task 03 (2026-07-01), run_01 amendment (2026-07-04).**
...
| R6 | Spaces ↔ Facilities (instances) | M:N (via Space_Facilities) | both partial | outputs/01 §3.2 |
...
| R12 | Spaces → Incidents | 1:N | Incidents total (every incident belongs to exactly one space) | run_01 amendment |
| R13 | Users → Incidents (reporter) | 1:N | Incidents total (every incident has a reporter) | run_01 amendment |
| R14 | Users → Incidents (assigned_to) | 1:N (partial) | Incidents partial (assignee may be set later) | run_01 amendment |
| R15 | Facility_Categories → Facilities | 1:N | Facilities total (each facility belongs to exactly one category) | run_01 amendment |
| R16 | Facilities → Maintenance | 1:N (partial) | Maintenance partial (only some maintenance targets a specific device) | run_01 amendment |
| R17 | Facilities → Incidents | 1:N (partial) | Incidents partial (only some incidents involve a specific device) | run_01 amendment |
...
**Description:** Lookup table mapping category names to their 3-letter prefix used in auto-generated facility codes.
**Maps to table:** `facility_categories`
**Source:** run_01 amendment
...
| category_name | VARCHAR(50) | NO | UQ | UNIQUE | e.g. projector, whiteboard, microphone, computer, livestreaming_equipment, air_conditioner |
| prefix | CHAR(3) | NO | UQ | UNIQUE | 3-letter code used in facility_id generation |
| created_at | DATETIME2 | NO | — | — | DEFAULT GETDATE() |
...
**Description:** Individual physical device instances in inventory (one row per device); facility code derived as `prefix_facility_id` at query time.
**Maps to table:** `facilities`
**Source:** outputs/01 §3.2 (refactored run_01 amendment)
...
| category_id | INT | NO | FK | `FK → facility_categories.category_id` | Which category this device belongs to |
| status | VARCHAR(20) | NO | — | `CHECK IN ('active','inactive','retired','lost')` | DEFAULT 'active' |
| created_at | DATETIME2 | NO | — | — | DEFAULT GETDATE() |
| updated_at | DATETIME2 | NO | — | — | DEFAULT GETDATE() |

**Derived attribute:** `facility_code` = `CONCAT(fc.prefix, '_', f.facility_id)` — computed at query time via JOIN to `facility_categories`. For example, `pro_1` (projector #1), `whb_10` (whiteboard #10).

---

### Space_Facilities

**Description:** Junction resolving the many-to-many between spaces and facility instances — one row per physical device assigned to a space.
...
**Note:** `quantity` column removed — each row represents one physical device instance. Quantity per category is derived via `COUNT(*) GROUP BY space_id, category_id`.
...
| facility_id | INT | YES | FK | `FK → facilities.facility_id` | Specific device being repaired (R16); nullable (maintenance may target the whole space) |
...
**Description:** Unexpected events (theft, vandalism, accidents, safety hazards) reported against a space or specific device, with trigger-based auto-resolution when related maintenance completes.
...
| facility_id | INT | YES | FK | `FK → facilities.facility_id` | Specific device involved (R17); nullable (incident may be about the space as a whole) |
...
| Date | Change | Reason |
|---|---|---|
| 2026-07-04 | run_01 facility refactoring — added Facility_Categories entity, refactored Facilities to instance-level, added facility_id FK to Maintenance/Incidents, added R15–R17 | Facility instance-level tracking + prefix lookup |
